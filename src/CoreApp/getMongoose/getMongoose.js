...
export default async (ctx, params) => {
  ...
  ctx.log.trace('db init2');

  const testSchema = new  mongoose.Schema({
      test:       { type: String, required: false },
  });
  ctx.log.trace('db init3');

  const Test = mongoose.model('Test', testSchema);
  ctx.log.trace('db init4');

  const test = new Test({
    test: 'a b c d'
  })
  ctx.log.trace('db init5', test);

  const promise = test.save(() => {
    console.log('saved');
  })
  ctx.log.trace('db init promise', promise);
  // console.log();
  await promise.then(() => {
    console.log('then');

  });
  ctx.log.trace('db init6');
