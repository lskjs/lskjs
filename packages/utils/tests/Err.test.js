...
test('Err("login_error", { realPassword: "password", data: { password: "incorrect" } })', () => {
  const input = ['login_error', 'Password didnt match', { realPassword: 'password', data: { password: 'incorrect' } }];
  const res = {
    name: 'Err',
    code: 'login_error',
  ...
  expect(err).toMatchObject(res);
  expect(err.getJSON()).toStrictEqual(res);
  expect(JSON.stringify(err)).toStrictEqual(JSON.stringify(pick(res, ['name', 'code', 'message', 'test', 'data'])));
});
