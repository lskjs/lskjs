...
//   "tls": true
// }
export default ctx => class MailerParserModule {
  async init() {
    // parser
    this.prefix = 'mailer';
    ...
    await Promise.mapSeries(mailboxes, async (mailbox) => {
      ...
      await Promise.mapSeries(boxes, async (box) => {
        ...
        try {
          ...
          console.error('sync err', err);  //eslint-disable-line
        }
        this.disconnect({ connection });
        if (boxDelay) {
  ...
  disconnect({ connection }) {
    ...
    return new Promise((resolve, reject) => connection.end((err, res) => {
      connection.removeAllListeners();
      if (err) {
        console.error('imap disconnect', err);  //eslint-disable-line
        return reject(err);
      }
      return resolve(res);
    }));
  ...
  async getBoxes(imapConfig) {
    ...
    connection.connect();
    return new Promise((resolve, reject) => {
      connection.once('ready', () => connection.getBoxes((err, boxes) => { // На всякий случай
        if (err) {
  ...
  async sync({
    ...
    try {
      ...
      connection.removeAllListeners();
      console.error('searchAndSave', err);  //eslint-disable-line
    }
  }
      ...
      try {
        ...
        return connection.search(filter, async (searchErr, results) => {
          ...
          const emails = await Email
            ...
            .select(['uid'])
            .lean();
          results = results.filter(uid => !find(emails, { uid }));  //eslint-disable-line
          if (results.length > this.config.limit) {
            results = results.slice(0, this.config.limit);  //eslint-disable-line
          }
          if (!results.length) return resolve([]);
          connection.on('error', (err) => {
            ...
            try {
              ...
              if (received) {
                if (Array.isArray(received)) {
                  received = received[0];  //eslint-disable-line
                }
                if (received) {
              ...
              if (from) {
                message.from = from.value[0];  //eslint-disable-line
              }
              if (to) {
                message.to = to.value[0];  //eslint-disable-line
              } else {
                const deliveredTo = headers.get('delivered-to');
                if (deliveredTo && deliveredTo.value && deliveredTo.value[0]) {
                  message.to = deliveredTo.value[0];  //eslint-disable-line
                }
              }
              if (attrs['x-gm-thrid']) {
