...
export function copyStaticProperties(base: Record<string, unknown>, target: Record<string, unknown>): void {
...
export function inject(/* fn(stores, nextProps) or ...storeNames */ ...storeNames: Array<any>) {
  ...
  return (componentClass: React.ComponentClass<any, any>) =>
    createStoreInjector(grabStoresByName(storeNames), componentClass, storeNames.join('-'), false);
}
