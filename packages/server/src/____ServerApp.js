...
export default class ServerApp extends Module {
  ...
  async init() {
    await super.init();
    if (this.debug) this.log.trace('ServerApp init');
    ...
    if (this.debug) this.log.debug('serverConfig.middlewares', this.serverConfig.middlewares);
  ...
  async model(nameOrNames) {
    ...
    if (!this.models || !this.models[name]) throw new Err('MODULE_MODEL_MISSING', { data: { name } });
  ...
  async runWs() {
    ...
    if (this.debug) this.log.trace('ServerApp.runWs');
  ...
  runMiddlewares() {
    if (this.debug) this.log.trace('ServerApp.runMiddlewares');
  ...
  runDefaultRoute() {
    if (this.debug) this.log.trace('ServerApp.runDefaultRoute');
  ...
  async run(...args) {
    ...
    if (this.debug) this.log.trace('ServerApp.run');
  ...
  async afterRun() {
    if (this.debug) this.log.trace('ServerApp afterRun');
