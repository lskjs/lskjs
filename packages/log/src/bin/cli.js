...
Object.keys(levelFromName).forEach(function (name) {
    ...
    upperPaddedNameFromLevel[lvl] = '[' + name[0] + ']'
...
function emitRecord(rec, line, opts, stylize) {
    ...
    case OM_LONG:
        ...
        if (!short)
          ...
          if(['app/req', 'uapp/req'].filter(name => nameStr.substr(0, name.length).toLowerCase()=== name.toLowerCase()).length) {
          // if(nameStr.substr(0, 'app/req'.length) === 'app/req') {
            emit(format('%s%s\n',
                level,
                onelineMsg));
          } else if(['app', 'uapp'].filter(name => nameStr.substr(0, name.length).toLowerCase()=== name.toLowerCase()).length) {
            // if(nameStr.length == 3) {
              emit(format('%s %s:%s%s\n%s',
                  level,
                  nameStr,
