...
export abstract class ModuleWithLifecycle implements IModuleWithLifecycle {
  ...
  static async create<T extends IModule>(
    this: IModuleConstructor<T>,
    ...propsArray: IModuleProps[]
  ): Promise<T> {
    const instance = new this();
  ...
  static async createAndRun<T extends IModule>(
    this: IModuleConstructor<T>,
    ...propsArray: IModuleProps[]
  ): Promise<T> {
  ...
  static async start<T extends IModule>(
    this: IModuleConstructor<T>,
    ...propsArray: IModuleProps[]
  ): Promise<T> {
  ...
  async __run(): Promise<void> {
    ...
    if (!this.__lifecycle.initFinish) {
      throw new Err(
        'MODULE_INVALID_LIVECYCLE_INIT_WAIT',
