...
const processOrNull = isClient ? null : process;
// @ts-ignore
const processEnvOrWindow = isClient ? window : process.env;

export const isColorSupported =
  !(
    'NO_COLOR' in processEnvOrWindow ||
    (isServer && process.argv.includes('--no-color'))
  ) &&
  (isClient ||
...
export function formatter(open: any, close: any, replace = open) {
  return (input: any) => {
