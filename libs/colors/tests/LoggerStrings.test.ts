...
describe('Logger strings', () => {
  ...
  test('req GET2', () => {
    ...
    expect(prettyFormat(data)).toMatchObject(['[d] ♦ GET /api/healthcheck     #8kpzlhVFUzihr-gdYutvT 200   3ms 295B']);
  });
  test('req GET3', () => {
    const data = {
      time: null,
    ...
    expect(prettyFormat(data)).toMatchObject(['[d] ♠ GET /api/healthcheck     #yEoDMODUXnCYx2piYdia9 200  22ms 511B']);
  });
  test('req GET4', () => {
    const data = {
      time: null,
    ...
    expect(prettyFormat(data)).toMatchObject([
      '[d] § GET /youtube/channels/123/products/321 #2srlW-ycUSEJyQE8U8tZk 200    1s 112.6kiB',
    ]);
  });
  test('req GET5', () => {
    const data = {
  ...
  test('req GET6', () => {
    ...
    expect(prettyFormat(data)).toMatchObject([
      '[d] ○ GET /youtube/sites/123/videos                                            #gJFUQ9C6-NNYM_VrU0UVq 200    2s 152.3kiB',
    ]);
  });
});
